...
if($_SESSION[username]=="")
{
    ...
    $query="select * from ".$tblpref."media "; 
	if(!($result=mysqli_query($connection,$query))){ echo $query.mysqli_errno($connection); exit;}
		   ...
		   $typev=stripslashes($txtname);
		   if($txtname!="")
		   {
				$nm = addslashes($typev);
				$condition[]="med_pub ='$nm'";
		   ...
		   if(is_array($condition))
			{
			 ...
			 $que="SELECT * FROM  ".$tblpref."media  $condition $condition2"; 
...
<table cellspacing="3" cellpadding="0" border="0" width="100%" align="center"  valign="top" class="tbborder">
	<tr><td align="center" ><b><h2>Media Publishers List</h2></b></td></tr>
    <tr>
    <td valign="top" width="75%">
				...
				<option value="<?php  echo $row[med_pub]?>" <?php if($row[med_pub]==$typev){?>selected <?php }?>><?php  echo $row[med_pub]?> </option>
...
<?php  } ?>	
	...
	<tr>
	<td align="right">
	<a href="pub-add.php?mode=add">ADD NEW</a> 
	</td>
	</tr>
	<tr>
	<td align="right">
	...
	<th align="center">
		...
		<table class="tbborder"	cellspacing="1" cellpadding="2" border="0" width="100%">
		   <tr>
			<th>Sr.no</th>
			<th><a href="?sorton=med_pub&sortby=<?php 
			if($_GET[sorton]=="med_pub " && ($_GET[sortby])=="desc") 
				echo "asc";
			    else
				echo "desc";
				?>" class="nav">Publisher Name</th>
				<th>Type</th>
			<th>Action</th>
		</tr>
		<?php 
		while($row_category=mysqli_fetch_array($page_res)){ 
			$id=$row_category[med_id];
			$count ++;
			
			?>
			<tr width="100%">
			<td width="5%" class="tbborder"><?php  echo $count?></td>
			<td  width="45%"class="tbborder"  align="left"><?php  echo stripslashes($row_category[med_pub])?></td>
			<td class="tbborder"  align="left"><?php  echo stripslashes($row_category[med_type])?></td>

			
			<td width="15%" class="tbborder" align="center"><a href="pub-add.php?mode=edit&mid=<?php  echo $id?>" > Edit</a>&nbsp; |&nbsp;<a href="submit-pub-media.php?mid=<?php  echo $id?>&mode=del&fn=
			<?php  echo $row_category[med_pub_logo]?>" class="menu" onclick='if(confirm("Do You Want To Delete This ?")){return true;}else{return false;}'> Delete</a> </td>
