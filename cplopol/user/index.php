...
$query="select * from  ".$tblpref."admin where username!='admin' order by admin_id DESC"; 
...
if ($_GET[sorton]!="")
		...
		{
			$condition2=" ORDER BY admin_id DESC";
		}
		$typev=$txtname;
		if($txtname!="")
		{
			$condition[]="username='$txtname'";
		...
		$condition[]="username!='admin'";
			...
			$que="SELECT * FROM  ".$tblpref."admin $condition $condition2"; 
							...
							$srnum=explode(",",$srnum);
							$count=$srnum[0];
admin_header('../../','Admin User List',$tblpref,$db,$sitepath,$siteurl,$path1,$ckpath,$row_admin);
?>
          ...
          <tr>
            ...
            <td width="60%">
			  <select NAME="txtname"  class="inpt" >
				...
				{?>
				<option value="<?php  echo $row[username]?>" <?php if($row[username]==$typev){?>selected <?php }?>><?php  echo $row[username]?> </option>
...
<div class="box">
    <div class="hdr">
		<h1 class="ico"><img src="icon/icon.png" alt="">Admin User List</h1>
        <div class="addnew">
				<?php 
				if($_SESSION[user_type]=="superadmin")
				{ ?>
						<a  href="user_add.php?mode=add" class="add">Add New</a>
  ...
  <!--   <p align="right"></p> -->
	...
	<?php 
		...
		$clcnt++;
		$id=$row_country[admin_id];
		$count++;		
		?>
	
		<tr class="<?=$class?>">
		<td><b><?=$count?></b></td>
		<td><?=stripslashes($row_country[username])?></td>
		<td><?=stripslashes($row_country[admin_name])?></td>
		<td><?=stripslashes($row_country[user_type])?></td>
		<td class="center">
		<ul class="actions">
			<li><a title="edit" href="user_add.php?mode=edit&id=<?php  echo $id?>" ><img alt="edit" src="../../images/pencil.png"></a></li>
